...
Outputs (written to --output-dir, default: semantic_output/):
  data/
    ...
    epoch_relative_drift.csv  — epoch-to-epoch relative drift
...
Dependencies:
  pip install sentence-transformers nltk textblob matplotlib pandas scipy
...
def sentiment_score(caption: str) -> tuple[float, str]:
    """
    Sentiment scoring using VADER (preferred for descriptive/social text),
    falling back to TextBlob, then to a simple lexicon count.
    Returns (score, label) where score is in [-1, 1] and label is
    'positive', 'negative', or 'neutral'.
    """
    # Try VADER first — better for descriptive, mixed-register text
    try:
...
def compute_relative_epoch_drift(
    ...
    """
    Computes epoch-to-epoch semantic drift.
...
def fig_sentiment_trajectory(iter_df: pd.DataFrame, out_path: Path):
    ...
    Color-coded by category, with non-overlapping end labels.
    """
    plt.style.use(FIGURE_STYLE)
    ...
    # Determine y range for label adjustment
    all_sent = iter_df.groupby(["seed_name", "iteration"])["sentiment_score"].mean()
    y_lo = all_sent.min() - 0.05
    y_hi = all_sent.max() + 0.05
    label_info = adjust_label_positions(label_info, min_gap=0.018,
...
def fig_sentiment_faceted(iter_df: pd.DataFrame, out_path: Path):
    """
    Faceted per-seed sentiment trajectory showing individual chain lines
    with a bold mean overlay. Reveals per-chain sentiment divergence.
    """
    seeds    = sorted(iter_df["seed_name"].unique())
    n_seeds  = len(seeds)
    n_cols   = 3
    ...
    for ax, seed_name in zip(axes_flat, seeds):
        ...
        # Individual chain lines
        for chain_num, cdf in sdf.groupby("chain_num"):
            chain_sent = cdf.sort_values("iteration")
            ax.plot(chain_sent["iteration"], chain_sent["sentiment_score"],
                    color=color, alpha=0.25, linewidth=0.8)

        # Bold mean line
        mean_sent = sdf.groupby("iteration")["sentiment_score"].mean()
        ax.plot(mean_sent.index, mean_sent.values,
    ...
    fig.suptitle(
        "Per-Seed Sentiment Trajectories (individual chains + mean)",
        fontsize=13, fontweight="bold", y=1.01,
    )
    plt.tight_layout()
    plt.savefig(out_path, dpi=FIGURE_DPI, bbox_inches="tight")
...
def fig_refusal_timeline(iter_df: pd.DataFrame, out_path: Path):
    """
    Scatter plot: seeds × iterations, marker darkness = fraction of chains
    with refusal language at that iteration. Darker = more chains affected.
    """
    # Compute refusal rate per seed per iteration (across chains)
    refusal_rate = (
    ...
    # Only show seeds that had at least one refusal
    has_refusal = refusal_rate.loc[refusal_rate.max(axis=1) > 0]
    if has_refusal.empty:
        print("  Skipping refusal timeline (no refusals detected).")
        return

    # Sort by first refusal iteration
    first_refusal_iter = has_refusal.apply(
        lambda row: row[row > 0].index.min() if (row > 0).any() else 999,
        axis=1,
    )
    has_refusal = has_refusal.loc[first_refusal_iter.sort_values().index]

    plt.style.use(FIGURE_STYLE)
    fig, ax = plt.subplots(
        figsize=(max(10, len(has_refusal.columns) * 0.7),
                 max(4, len(has_refusal) * 0.5 + 1))
    )

    iterations = has_refusal.columns.tolist()
    seeds      = has_refusal.index.tolist()

    for i, seed in enumerate(seeds):
        for j, it in enumerate(iterations):
            rate = has_refusal.loc[seed, it]
            if rate > 0:
                # Gradient: alpha scales with fraction of chains that had refusals
                # rate=0.2 (1/5 chains) -> light, rate=1.0 (5/5) -> full dark
                ax.scatter(j, i, s=120, c="red",
                           alpha=max(0.2, rate), zorder=3,
    ...
    ax.set_title(
        ...
        "Darker = more chains with refusal language at that iteration",
        fontsize=12, fontweight="bold",
    )

    # Add a manual legend for the gradient
    from matplotlib.lines import Line2D
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor='red',
               alpha=0.2, markersize=8, label='1/5 chains'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor='red',
               alpha=0.6, markersize=8, label='3/5 chains'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor='red',
               alpha=1.0, markersize=8, label='5/5 chains'),
    ]
...
def fig_word_bump_charts(word_freq_df: pd.DataFrame,
                         epoch_size: int, out_dir: Path):
    """
    Per-seed bump chart tracking the top 10 words by rank across epochs.

    Y-axis = rank (1 at top, 10 at bottom). X-axis = epoch.
    Lines connect each word across epochs.

    Colors:
      Green  — word present in epoch 1 and persists
      Red    — word absent from epoch 1, appears later (insertion/mutation)
      Gray   — word exits the top 10 (line ends, final point grayed out)

    Labels appear once at the word's first ranked epoch. Epoch 1 labels
    are placed on the left with collision avoidance; later labels on the right
    of the point.
    ...
    for seed_name in seeds:
        ...
        # Build per-epoch rank tables
        # For each epoch, take top_n words, rank by count desc then alpha asc for ties
        epoch_ranks: dict[int, dict[str, int]] = {}
        epoch1_words: set[str] = set()

        for epoch in epochs:
            ep_df = (
                seed_df[seed_df["epoch"] == epoch]
                .sort_values(["count", "word"], ascending=[False, True])
                .head(top_n)
            )
            ranked = {}
            for rank, (_, row) in enumerate(ep_df.iterrows(), start=1):
                ranked[row["word"]] = rank
            epoch_ranks[epoch] = ranked
            if epoch == epochs[0]:
                epoch1_words = set(ranked.keys())

        # Collect all words that appear in any epoch's top N
        all_words = set()
        for ranks in epoch_ranks.values():
            all_words.update(ranks.keys())

        if not all_words:
            continue

        # Classify each word
        word_class: dict[str, str] = {}  # "stable", "inserted", "exited"
        for word in all_words:
            present_epochs = [ep for ep in epochs if word in epoch_ranks[ep]]
            if word in epoch1_words:
                word_class[word] = "stable"
            else:
                word_class[word] = "inserted"

        # Color mapping
        color_map = {"stable": "#16a34a", "inserted": "#dc2626"}
        gray = "#b0b0b0"

        plt.style.use(FIGURE_STYLE)
        fig, ax = plt.subplots(figsize=(3 + 2.5 * len(epochs), 6))

        # Track which words have been labeled already
        labeled: set[str] = set()
        ...
        for word in sorted(all_words):
            ...
            # Build trajectory: list of (epoch_idx, rank) where word appears
            trajectory = []
            for ep in epochs:
                if word in epoch_ranks[ep]:
                    trajectory.append((ep, epoch_ranks[ep][word]))

            if not trajectory:
                continue

            # Draw connecting lines between consecutive appearances
            ep_vals  = [t[0] for t in trajectory]
            rank_vals = [t[1] for t in trajectory]

            # Check if word exits before the last epoch
            last_epoch_present = ep_vals[-1]
            exits = last_epoch_present < epochs[-1]

            # Draw the main line
            ax.plot(ep_vals, rank_vals, color=color, linewidth=1.8,
                    alpha=0.8, zorder=2)

            # Draw points
            for ep, rank in trajectory:
                is_exit_point = exits and ep == last_epoch_present
                pt_color = gray if is_exit_point else color
                ax.scatter(ep, rank, color=pt_color, s=40, zorder=3,
                           edgecolors="white", linewidths=0.5)

            # If it exits, draw a faded marker at the exit point
            if exits:
                ax.scatter(last_epoch_present, trajectory[-1][1],
                           color=gray, s=50, zorder=4, marker="x", linewidths=1.5)

            # Label at first appearance only
            if word not in labeled:
                first_ep, first_rank = trajectory[0]
                labeled.add(word)
                # Epoch 1 labels go on the left; later insertions on the right
                if first_ep == epochs[0]:
                    ax.text(first_ep - 0.15, first_rank, word,
        ...
        ax.set_xticklabels(
            [epoch_label(ep, epoch_size) for ep in epochs],
            fontsize=9,
        )
        ax.invert_yaxis()  # rank 1 at top
        ax.set_xlabel("Epoch", fontsize=11)
        ax.set_ylabel("Rank (by frequency)", fontsize=11)
        ax.set_title(f"{seed_name} — Word Prominence by Epoch",
                     fontsize=13, fontweight="bold")
        ax.set_xlim(epochs[0] - 0.6, epochs[-1] + 0.4)

        # Legend
        from matplotlib.lines import Line2D
        legend_els = [
            Line2D([0], [0], color="#16a34a", linewidth=2, label="Persistent"),
            Line2D([0], [0], color="#dc2626", linewidth=2, label="Inserted"),
            Line2D([0], [0], marker="x", color=gray, linewidth=0,
                   markersize=6, label="Exited top 10"),
        ]
        ax.legend(handles=legend_els, fontsize=8, loc="lower right")

        plt.tight_layout()
        safe_name = seed_name.replace(" ", "_").replace(".", "")
        out_path  = out_dir / f"bump_{safe_name}.png"
        plt.savefig(out_path, dpi=FIGURE_DPI, bbox_inches="tight")
        plt.close()

    print(f"  Saved: word bump charts in {out_dir.name}/")
...
def main():
    ...
    fig_sentiment_faceted(iter_df, figs_out / "sentiment_faceted.png")
    ...
    fig_word_bump_charts(word_freq_df, epoch_size, figs_out / "word_bump_charts")
