...
def adjust_label_positions(labels: list[dict], min_gap: float = 0.025,
                           y_min: float = -0.2, y_max: float = 1.0,
...
def fig_face_individual_decay(scores_df: pd.DataFrame, out_path: Path):
    ...
    label_info = adjust_label_positions(label_info, min_gap=0.028,
                                        y_min=-0.2, y_max=1.0)
    ...
    ax.axhline(0.3, color="red",  linestyle="--", linewidth=0.9, alpha=0.6)
    ax.axhline(0.0, color="gray", linestyle="--", linewidth=0.6, alpha=0.4)
